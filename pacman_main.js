    ...
    .toArray((err, result) => {
      if (err || result[0] == undefined) {
        ...
        if (bcrypt.compareSync(password, result[0].password)) {
          response.cookie("username", [username, result[0].highscore]);
...
app.post("/submit", (request, response) => {
  ...
  console.log("Score: " + score + " Second: " + second + " Corner: " + corner + " Progression: " + request.body.progression)
  db.collection(USERS_COLLECTION)
    .findOne({
      username: "Death"
    })
